...
FROM `MAPAUBA`.`geodatos`
left outer JOIN MAPAUBA.actividades on `geodatos`.id_actividades = actividades.id
...
/*limit 100*/";
	...
	$query="SELECT distinct `geodatos`.id_actividades,
		...
		`geodatos`.`zz_bloqueadoTx`,
		`geodatos`.`zz_bloqueadoUsu`,
		z - $zoom + case when `geodatos`.id_usuarios = $UsuarioI then 0 else 3 end - (select count(a.id_p_TAGs_id) from MAPAUBA.ACTtags a where a.id_p_actividades_id = $actividad
		...
		and x >= $x0 and x <= $xf and y >= $y0 and y <= $yf
		".
		// lo que sigue restringe la búsqueda solo dentro del área de la actividad and exists (select 1 from MAPAUBA.actividades where actividades.id = $actividad and x >= actividades.x0 and x <= actividades.xf and y <= actividades.y0 and y >= actividades.yf)
		"and z >= $zoom and `geodatos`.id_actividades != $actividad
		order by ranking
		limit 100";
